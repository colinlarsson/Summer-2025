...
'Version 4.15 Last Update 2025-06-13
...
' --- Constants ---
Const PUMP_C_FCAL As Double = 15.92 ' Feed A gravimetric f.cal
Const PUMP_D_FCAL As Double = 24    ' Glucose gravimetric f.cal (Note: Script uses Pump A for Glucose)
Const PUMP_A_FCAL As Double = 105   ' Feed B volumetric f.cal (Note: Script uses Pump D for Feed B)
Const PHASE_DELAY_SECONDS As Double = 0.1 / 60 ' Small delay after phase change (e.g., 6 seconds)
Const SAMPLE_CONFIRM_DELAY_HOURS As Double = 4 ' Delay for sample confirmation
Const GLUCOSE_SAMPLE_WAIT_HOURS As Double = 8 ' Specific wait time for glucose sample confirmation
Const OVERFEED_PERCENTAGE As Double = 1.2 ' Max totalizer percentage before stopping pump (120%)
...
'p.logmessage(p.BalanceAState())

' BalanceAState(0) = inactive
' BalanceAState(1) = idle
' BalanceAState(2) = busy
' BalanceAState(3) = manual
' BalanceAState(4) = calibrate
' BalanceAState(5) = error

Dim State As Integer = p.BalanceAState()
'_____________________________________________________________________________________________________________

Dim Scale_Weight(5) as Double ' Adjusted array size to match usage (indices 1-5)
Scale_Weight(1) = p.MAPV 'p.MAPV 'time zero bottle weight 
Scale_Weight(2) = p.MAPV 'p.MAPV 'interval starting bottleweight 
Scale_Weight(3) = p.MAPV 'p.MAPV 'current weight feed A
Scale_Weight(4) = p.MAPV 'p.MAPV 'glucose interval starting bottleweight
Scale_Weight(5) = p.MAPV 'p.MAPV 'current weight glucose
...
Dim sampleDelay as Double = SAMPLE_CONFIRM_DELAY_HOURS 'Set to 2/60 for testing
...
'Feed B Target Volumes - pump D (as per script logic)
...
If s Is Nothing Then
    Dim a(12) As Double ' Adjusted array size to only include used indices (0-12)
    a(1) = 0 ' time zero bottle weight (initial value, updated later)
    a(2) = 0 ' interval starting bottleweight (initial value)
    a(3) = 0 ' current weight (initial value)
    a(4) = Major_Feed_TargetWeight ' Major Feed Target Weight
    a(5) = 0 ' Major Feed Counter (initialized to 0)
    a(6) = 0 ' Feed Start Time (initialized to 0)
    a(7) = 0 ' Feed B Counter (initialized to 0)
    a(8) = 0 ' Feed B Totalizer (initialized to 0)
    a(9) = 0 ' Feed B Volume Target (initialized to 0)
    a(10) = 0 ' Glucose interval starting bottleweight (initial value)
    a(11) = 0 ' current weight glucose (initial value)
    a(12) = 0 ' user input glucose target
    s = a
End If
...
With P 
    ...
    ' Ensure MajorFeed_Fed is always positive for comparison with target
    If MajorFeed_Fed < 0 Then
        MajorFeed_Fed = 0
    End If
...
' --- Scale State Logging Function ---
Call LogScaleState(State, lastState, P)
...
Select Case .phase
    Case 0 'Workflow Start 
        .logmessage("██████████████-----AUTOFEED v.4.17 LAUNCHING...")
        .phase = .phase + 1
...
'==========================================================================================
    ...
    If .FACal <> PUMP_C_FCAL Then .FACal = PUMP_C_FCAL
    If .FCCal <> PUMP_D_FCAL Then .FCCal = PUMP_D_FCAL ' This is Pump D, but used for Glucose in this script
    If .FBCal <> PUMP_A_FCAL Then .FBCal = PUMP_A_FCAL ' This is Pump A, but used for Feed B in this script

    If .InoculationTime_H > 0 And (.runtime_H - .phaseStart_H) > PHASE_DELAY_SECONDS Then
        .logmessage("██████████████----AUTOMATED FEEDING INITIALIZED FOR UNIT#: " & .unit & ". Pumps Calibrated ----")
        .phase = .phase + 1
    End If
...
'sits here prior to feed 1
    Case 2
        ...
        If .VCPV <> 0 Then .SetVCPV(0) ' Reset Pump C totalizer
        If .VAPV <> 0 Then .SetVAPV(0) ' Reset Pump A totalizer
        If .VDPV <> 0 Then .SetVDPV(0) ' Reset Pump D totalizer

        ' Ensure counters are integers
        s(5) = CInt(s(5)) ' Major Feed Counter
        s(7) = CInt(s(7)) ' Feed B Counter
        ...
        If .OfflineM = 0 Then 'Update Counters Manually, M: 1 enter reset state, N: number of steps to skip, O: step up 1. M will intercept after every feed 
            If .InoculationTime_H > Feed_Time(1) And (.runtime_H - .phaseStart_H) > PHASE_DELAY_SECONDS Then 'waits for first feed time + 6s delay; 
...
'==========================================================================================
    ...
    Case 3 'turn off pump and assign new values for interval time and specified target weight to feed
        ...
        ' Major Feed - pump C
        ' Use direct array access for efficiency and readability
        If s(5) >= 1 And s(5) <= 12 Then
            Feed_StartTime = Feed_Time(CInt(s(5)))
            Major_Feed_TargetWeight = Major_Feed_TargetWeight_Value(CInt(s(5)))
        Else
            ' Handle case where s(5) is out of expected range, e.g., log error or set default
            .logalarm("██████ Error: Major Feed Counter (s(5)) out of range: " & s(5) & " ██████")
            Feed_StartTime = 0 ' Or some default/error value
            Major_Feed_TargetWeight = 0 ' Or some default/error value
        End If

        ' Feed B | s(7) = feed B counter, s(8) = pump D totalizer, s(9) = Feed B volume target
        ' Use direct array access for efficiency and readability
        If s(7) >= 1 And s(7) <= 12 Then
            FeedB_Totalizer = .VDPV ' Current totalizer reading for Pump D (Feed B)
            FeedB_VolumeTarget = FeedB_VolumeTarget_Value(CInt(s(7)))
        Else
            ' Handle case where s(7) is out of expected range
            .logalarm("██████ Error: Feed B Counter (s(7)) out of range: " & s(7) & " ██████")
            FeedB_Totalizer = .VDPV ' Still get current totalizer
            FeedB_VolumeTarget = 0 ' Or some default/error value
        End If
    ...
    If procDay <= 0 Then ' First decision tree, skips sample confirmation on day 0
        .intT = s(6) - .inoculationTime_H 'time to feed in hours. (Note: .intT is not used later)
        .phase = .phase + 2
    'if sample not confirmed before feed time, will sit and wait in next phase. this logmessage runs a day early
    ElseIf .offlineA <> procDay And .inoculationTime_H < (s(6) + sampleDelay) And ((.runtime_H - .PhaseStart_H) > PHASE_DELAY_SECONDS) Then 'Waiting for offline A to confirm sample
...
'sits here after first feed, waiting for sample confirmation
    ...
    Case 4 'waiting for sample confirmation, goes to feed b once procDay is set
        If .offlineA <> procDay And .inoculationTime_H > (s(6) + sampleDelay) Then 'Waiting for offline A to confirm sample, and time has passed
...
'==========================================================================================
    ...
    If ((.runtime_H - .PhaseStart_H) > PHASE_DELAY_SECONDS) and .inoculationTime_H > s(6) and s(9) <> 0 Then '6 second delay to ensure values get stored to array. also accounts for hiccups in DWC, checks for fb entry
        .logmessage("Phase: 5.  Day: " & procDay & ".  Unit #: " & .unit & ".  ---Feed B Starting: " & s(9) & " [g] to be fed.")
        .phase = .phase + 1
    ElseIf ((.runtime_H - .PhaseStart_H) > PHASE_DELAY_SECONDS) and .inoculationTime_H > s(6) and s(9) <= 0 Then 'corrected condition to check for no feed
...
'==========================================================================================
    ...
    If s(7) <= 12 Then 'checks if above max feeds -probably not necessary
        ...
        .logmessage("██ - Phase: 6 - Day " & procDay & " - Unit #: " & .unit & ". - ██ - Feed B Finished: " & s(9) & " [g] Fed")
            .phase = .phase + 1
        End If
    ElseIf s(7) > 12 AndAlso ((.runtime_H - .PhaseStart_H) > PHASE_DELAY_SECONDS) Then '6 second delay to ensure values get stored to array. also accounts for hiccups in DWC
        .phase = 2 'finish him 
    End If 
...
'==========================================================================================
    ...
    Call LogScaleState(State, lastState, P)
        ...
        s(2) = Scale_Weight(2) ' Update interval starting bottleweight after Feed B

        If CInt(s(5)) > 12 Then ' Check if Major Feed Counter has exceeded max feeds
            .phase = 2 'finish him 
        Else
...
'==========================================================================================
    ...
    Call LogScaleState(State, lastState, P)
        ...
        If .InoculationTime_H > s(6) AndAlso s(4) <> 0 AndAlso (.runtime_H - .phaseStart_H) > PHASE_DELAY_SECONDS Then 'waits for respective feed time + 6s delay; 
            .phase = .phase + 1
...
'==========================================================================================
    ...
    Call LogScaleState(State, lastState, P)
        ...
        ' Ensure MajorFeed_Fed is calculated correctly for comparison
        MajorFeed_Fed = (s(2) - s(3))
        If MajorFeed_Fed < 0 Then MajorFeed_Fed = 0 ' Ensure positive value for comparison
        ...
        If MajorFeed_Fed >= s(4) And (.runtime_H - .phaseStart_H) > PHASE_DELAY_SECONDS Then
            .pumpCActive = False
            .logmessage("██ - Phase: 9 - Day " & procDay & " - Unit #: " & .unit & ". - ██ - Major Feed A Finished: " & formatNumber(MajorFeed_Fed, 2) & " [g]")
            .phase = .phase + 1
...
'==========================================================================================
    ...
    Case 10 'Glucose Decision Tree
        ...
        If procDay = 0 Then 'should never happen
            .logmessage("Phase: 10.  Day: " & procDay & ".  Unit #: " & .unit & ".  ---No Glucose addition needed on Day 0, proceeding to Phase 13.")
            .phase = .phase + 5
...
'==========================================================================================
    ...
    Case 11 'potential skip of glucose
        ...
        If .offlineA <> procDay and .inoculationTime_H > (s(6) + SAMPLE_CONFIRM_DELAY_HOURS) Then 'skips glucose if sample not confirmed
...
'==========================================================================================
    ...
    Call LogScaleState(State, lastState, P)
        ...
        If .InoculationTime_H > s(6) and (.runtime_H - .phaseStart_H) > PHASE_DELAY_SECONDS Then 'waits for respective feed time + 6s delay; 
            .phase = .phase + 1
        End If 
'==========================================================================================
...
'==========================================================================================
    ...
    Case 14 'glucose feed 
        If .runtime_H - .phaseStart_H < 0.025/60 Then 
            .logmessage("Phase: 14.  Day: " & procDay & ".  Unit #: " & .unit & ".  ---Sampling Confirmed: " & formatNumber(s(12), 2) & " [g] Glucose to be fed.")
        End If
    Call LogScaleState(State, lastState, P)
        s(11) = Scale_Weight(5) 'dynamic interval scale weight 
        .pumpAActive = True 'Glucose starts
        
        ' Ensure glucoseFeed_Fed is calculated correctly for comparison
        glucoseFeed_Fed = (s(10) - s(11))
        If glucoseFeed_Fed < 0 Then glucoseFeed_Fed = 0 ' Ensure positive value for comparison
        ...
        If abs(glucoseFeed_Fed) > s(12) and (.runtime_H - .phaseStart_H) > 0.1/60 or s(12) = 0 Then ' = recorded glucose value 
            .logmessage("██ - Phase: 14 - Day " & procDay & " - Unit #: " & .unit & ". - ██ - Glucose Feed Finished: " & formatNumber(glucoseFeed_Fed, 2) & " [g]")
...
'==========================================================================================
    ...
    Case 15
        .pumpAActive = False
        If .runtime_H - .phaseStart_H < PHASE_DELAY_SECONDS Then 
            .logwarning("██ - Phase: 15 - ██ - Day: " & procDay & " - ██ - Unit #: " & .unit & ". - ████ - Total Feeds: - ██ - Feed A: " & formatNumber(MajorFeed_Fed, 2) & " [g]. Feed B: " & formatNumber(s(9), 2) & " [mL]. Glucose:  " & formatNumber(glucoseFeed_Fed, 2) & "[g] - ██")
            .phase = 2
        End If
...
' --- Helper Subroutine for Scale State Logging ---
Static lastState As Integer ' Declared globally or as a static variable within the module
Sub LogScaleState(ByVal currentState As Integer, ByRef prevLastState As Integer, ByRef pObj As Object)
    If currentState <> prevLastState Then
        Select Case currentState
            Case 0 : pObj.logalarm("██████ Scale is inactive ██████")
            Case 1 : pObj.logalarm("██████ Scale is idle ██████")
            Case 2 : pObj.logMessage("██████ Scale is busy ██████")
            Case 3 : pObj.logalarm("██████ Scale is in manual mode ██████")
            Case 4 : pObj.logalarm("██████ Scale is calibrating... ██████")
            Case 5 : pObj.logalarm("██████ Scale has an error ██████")
        End Select
        prevLastState = currentState
    End If
End Sub
