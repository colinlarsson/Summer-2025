...
'Version 4.11 Working on 06-04-2025 
'Version for testing!
...
Major_Feed_TargetWeight_Value(1) = 50
Major_Feed_TargetWeight_Value(2) = 30
...
If State <> lastState Then
    Select Case State
        ...
        Case 3
            .logalarm("Scale is manual")
        Case 4
            .logalarm("Scale is calibrating")
        Case 5
            .logalarm("Scale has error")
    End Select
    lastState = State
End If
'_____________________________________________________________________________________________________________
...
Select Case .phase
    Case 0 'Workflow Start 
        .logmessage("██████████████-----AUTOFEED v.4.11 LAUNCHING...")
...
'Feed B | s(7) = feed B counter, s(8) = pump D totalizer, s(9) = Feed B volume target
    ...
    If procDay = 0 Then ' First decision tree, skips to feed b on day O
        .phase = .phase + 2
    ElseIf .offlineA <> procDay and .inoculationTime_H < (s(6) + sampleDelay) and ((.runtime_H - .PhaseStart_H) > 0.1/60)  'Waiting for offline A to confirm sample
    ...
    '_____________________________________________________________________________________________________________
	    ...
	    ElseIf s(4) = 0 Then 'if no feed A is entered, skip to glucose phase
            .logmessage("Phase: 4.  Day: " & procDay & ".  Unit #: " & .unit & ".  ---No Feed A, Skipping to Glucose Phase.")
		.phase = .phase + 2
        End If
    ...
    '_____________________________________________________________________________________________________________
        ...
        If abs(MajorFeed_Fed) > s(4) and (.runtime_H - .phaseStart_H) > 0.1/60 Then '(s(1) - s(2)) > s(12)
            .pumpCActive = False
            .logmessage("██ - Phase: 9 - Day " & procDay & " - Unit #: " & .unit & ". - ██ -  " & procDay & ":  " & formatNumber(MajorFeed_Fed, 2) & " [g]")
            .phase = .phase + 1
    ...
    Case 11 'potential skip of glucose
        ...
        If .offlineA <> procDay and .inoculationTime_H > (s(6) + sampleDelay) Then
            .logwarning("██ - Phase: 11 - ██ - Day: " & procDay & " - ██ - Unit #: " & .unit & ". - ████ - Phase 9: Skipping glucose feed, sample not verified")
            .phase = .phase + 3
    ...
    '_____________________________________________________________________________________________________________
        ...
        Else if (.runtime_H - .phaseStart_H) > 0.1/60 and .VAPV > (1.2 * s(12)) 'if dynamic totalizer is above 120% of target weight then stop
            .pumpAActive = False
            .logwarning("██ - Phase: 10 - ██ - Day: " & procDay & " - ██ - Unit #: " & .unit & ". - ████ - Totalizer: " & formatNumber(.VAPV, 2) & " [g] is above 120% of target weight: " & s(12) & " [g]. Stopping Feed A.")
            .Phase = .phase + 1
        End If
